#Graphs and metrics: 
#Correlation and time series of residential water consumption per day in Joinville with ARIMA
...
data <- tab_cor |>   
  select(Date, `liter/inhabitant/day`) |>   
  mutate(Month = yearmonth(Date)) |>      
  rename(Consumption = `liter/inhabitant/day`) |> 
...
fc |>  
  ...
  labs(y = "liter/inhabitant/day")

# Plot with forecast and confidence intervals
fc |>
  autoplot(train) +
  labs(title = fit$`ARIMA(Consumption)`, y = "liter/inhabitant/day") 
