#Graphs and metrics: 
#Correlation and time series of residential electricity consumption per day in Joinville with ARIMA
...
data <- tab_cor |>   
  select(Date, `kWh/inhabitant/day`) |>   
  mutate(Month = yearmonth(Date)) |>      
  rename(Consumption = `kWh/inhabitant/day`) |> 
...
fc |>  
  ...
  labs(y = "kWh/inhabitant/day")

# Plot with forecast and confidence intervals
fc |>
  autoplot(train) +
  labs(title = fit$`ARIMA(Consumption)`, y = "kWh/inhabitant/day") 
